/**
 * Compact Daraja (M-Pesa) error catalog + decoder.
 *
 * Vendored verbatim from paylod's own platform catalog
 * (`supabase/functions/_shared/daraja/error-catalog.ts`), which is the backend
 * twin of the web catalog rendered in the paylod dashboard + docs. It is MIT —
 * paylod's own data — and is copied here so the `decode_mpesa_error` MCP tool is
 * a PURE, offline lookup with zero network calls.
 *
 * KEEP IN SYNC: if the upstream STK result-code catalog changes, mirror it here.
 *
